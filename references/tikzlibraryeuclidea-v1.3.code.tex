  ...
  ellipse/.style args = {#1,#2,#3}{
    insert path = {
      let
        ...
        \p{PF2} = ($(#3) - (#2)$),
        \p{O} = ($(#1)!.5!(#2)$),
        \n{a} = {abs(veclen(\p{PF2}) + veclen(\p{PF1}))/2},
        \n{c} = {veclen(\p{F1F2})/2},
        \n{b} = {sqrt(\n{a}^2-\n{c}^2)},
        \n{angle} = {atan2(\y{F1F2}, \x{F1F2})},
      in [transform={\n{angle}:(\x{O},\y{O})}]
  ...
  hyperbola/.style args = {#1,#2,#3}{
    insert path = {
      ...
      in [smooth,domain=\pgfkeysvalueof{/tikz/hyperbola/@domain},variable=\euclidea@var,transform={\n{angle}:(\x{O},\y{O})}]
        plot({-\n{a}*cosh(\euclidea@var)},{\n{b}*sinh(\euclidea@var)})  % right arm
        plot({ \n{a}*cosh(\euclidea@var)},{\n{b}*sinh(\euclidea@var)})  % left arm
        plot({-\n{a}*sinh(\euclidea@var)},{\n{b}*sinh(\euclidea@var)})  % right arm
        plot({ \n{a}*sinh(\euclidea@var)},{\n{b}*sinh(\euclidea@var)})  % left arm
    }
  }, 
